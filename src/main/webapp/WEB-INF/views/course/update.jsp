...
document.addEventListener('DOMContentLoaded', function(){
	...
	document.querySelector('#sipNode-add-btn').addEventListener('click', e => {
		...
		let position = JSON.parse(JSON.stringify(cy.nodes(":selected")[0].position()));
		position.x+= 50;
		position.y-= 50;
		...
		node.animate({position}, {duration : 500});
		cy.zoom(1.5);
		cy.center(cy.nodes(":selected")[0]);
		cy.add({
			group: 'edges',
	...
	let updateShape = function(id, shape) {
		if(cy.nodes(':selected').length > 0) {
	    	...
	    	let position = JSON.parse(JSON.stringify(cy.getElementById('Main').position()));
	    	position.x += 30;
	    	position.y -= 30;
			node.animate({position}, {duration : 500});
			cy.zoom(1.5);
			cy.center(cy.getElementById('Main'));
		    return;
		}
	}
